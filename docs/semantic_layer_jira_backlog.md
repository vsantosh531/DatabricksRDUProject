# Enterprise Semantic Layer Platform — Jira Backlog (Full Detail)
...
teams across dev/staging/prod, proven via a fast end-to-end demo (Epic 0),
then hardened to production.
...
**Description:** Expand from the demo's single catalog to real
dev/staging/prod isolation, with catalog-to-workspace bindings enforcing
boundaries.

**Acceptance Criteria (epic exit criteria):**
- Dev, staging, and prod workspaces exist and are attached to the metastore.
- Catalogs are bound so prod is invisible from dev/staging and vice versa.
- Topology decision is documented and agreed by the platform owner.

**Checklist:**
- [ ] Topology decision documented
- [ ] Staging workspace provisioned
- [ ] Prod workspace provisioned
...
### E1-S2: Provision/confirm staging workspace
**Type:** Story | **Epic Link:** E1 | **Points:** 2 | **Priority:** High
**Labels:** `platform`, `workspace`, `terraform`
**Depends on:** E1-S1 | **Links:** blocks E1-S4, E9-S2

**Description:** Provision (or confirm existing) staging workspace, attached
to the shared metastore.

**Acceptance Criteria:**
- Workspace exists, attached to metastore, admin access confirmed.

**Checklist:**
- [ ] Workspace provisioned/confirmed
- [ ] Metastore attachment verified
- [ ] Admin access tested

### E1-S3: Provision/confirm prod workspace
**Type:** Story | **Epic Link:** E1 | **Points:** 2 | **Priority:** High
**Labels:** `platform`, `workspace`, `terraform`
**Depends on:** E1-S1 | **Links:** blocks E1-S4, E9-S1

**Description:** Provision (or confirm existing) prod workspace, attached
to the shared metastore.
...
**Labels:** `platform`, `unity-catalog`, `security`
**Depends on:** E1-S2, E1-S3 | **Links:** blocks E2-S6, E2-S7, E4-S1, E9-S1, E9-S2

**Description:** Bind each catalog to only the workspaces allowed to see
it (prod catalog bound to prod workspace only, etc.), enforcing the
isolation boundary grants alone can't provide.

**Acceptance Criteria:**
- Prod catalog is verifiably invisible from dev/staging workspaces.
- Binding configuration is captured in Terraform, not done via console only.

**Checklist:**
- [ ] Bindings configured per environment
- [ ] Cross-environment visibility tested and confirmed blocked
...
IaC-driven bundle template and Terraform modules that every domain team
scaffolds from.

**Acceptance Criteria (epic exit criteria):**
- A bundle template exists and successfully scaffolds a new project.
- Terraform modules manage workspace/catalog/schema config, not console
  clicks.
- Existing access grants are codified, not just manually configured.

**Checklist:**
- [ ] Terraform project structured
- [ ] Workspace and catalog/schema modules authored
...
### E2-S2: Author Terraform modules for workspace creation/config
**Type:** Story | **Epic Link:** E2 | **Points:** 5 | **Priority:** Medium
**Labels:** `iac`, `terraform`, `workspace`
**Depends on:** E2-S1 | **Links:** blocks E2-S5, E4-S7

**Description:** Reusable Terraform module(s) for workspace-level
configuration, parameterized per environment.

**Acceptance Criteria:**
- Module applies without manual console changes required afterward.
- Reused successfully across at least two environments (dev + staging).

**Checklist:**
- [ ] Module authored and parameterized
- [ ] Applied to dev
- [ ] Applied to staging
- [ ] Documented usage/variables

### E2-S3: Author Terraform modules for catalog/schema management
**Type:** Story | **Epic Link:** E2 | **Points:** 3 | **Priority:** Medium
**Labels:** `iac`, `terraform`, `unity-catalog`
**Depends on:** E2-S1 | **Links:** blocks E2-S4, E2-S5

**Description:** Reusable Terraform module(s) for catalog/schema creation
per domain/environment.

**Acceptance Criteria:**
- Module creates catalog + domain schemas matching the topology decision
  (E1-S1).
...
### E2-S4: Codify existing access grants as Terraform
**Type:** Story | **Epic Link:** E2 | **Points:** 3 | **Priority:** Medium
**Labels:** `iac`, `terraform`, `access`
**Depends on:** E2-S3 | **Links:** none

**Description:** Since access is already provisioned, capture the existing
grant model as Terraform for repeatability — reuse, not rebuild.

**Acceptance Criteria:**
- `terraform plan` against existing grants shows no unintended diff.
- Grant structure matches the layered model (domain schema → consumer
  `SELECT` on views only).

**Checklist:**
- [ ] Existing grants inventoried
- [ ] Grants expressed as Terraform
- [ ] `plan` shows clean/expected diff only
...
- [ ] Staging target defined and validated
...
**Description:** Add the validate → staging → prod promotion pipeline with
approval gates that the demo skipped.
...
**Checklist:**
- [ ] PR validate stage built
- [ ] Staging auto-deploy built
- [ ] Prod deploy with approval gate built
- [ ] Smoke tests wired in

### E3-S1: Build PR validate stage
**Type:** Story | **Epic Link:** E3 | **Points:** 5 | **Priority:** High
**Labels:** `ci-cd`, `testing`
**Depends on:** E2-S6 | **Links:** blocks E3-S2, E3-S3, E8-S1, E8-S5
...
### E3-S3: Build staging auto-deploy on merge
**Type:** Story | **Epic Link:** E3 | **Points:** 5 | **Priority:** High
**Labels:** `ci-cd`
**Depends on:** E3-S1 | **Links:** blocks E3-S4, E3-S6

**Description:** On merge to main: deploy to staging — UC layer (metric
view + grants) and workspace layer (Genie space + dashboards) as two
distinct steps.

**Acceptance Criteria:**
- Merge triggers staging deploy automatically.
- Both UC-layer and workspace-layer objects are confirmed updated.

**Checklist:**
- [ ] Staging deploy step added
- [ ] UC-layer deploy verified
- [ ] Workspace-layer deploy verified

### E3-S4: Implement smoke tests
**Type:** Story | **Epic Link:** E3 | **Points:** 3 | **Priority:** High
**Labels:** `ci-cd`, `testing`
**Depends on:** E3-S3 | **Links:** blocks E3-S5, E8-S4

**Description:** After staging deploy, query every measure at every
declared grain to catch broken definitions before prod.
...
- [ ] Wired into staging deploy step
...
**Acceptance Criteria:**
- Schema exists in dev/staging/prod catalogs per topology.
...
**Labels:** `network`, `security`
**Depends on:** E1-S3 | **Links:** blocks E9-S3
...
### E9-S2: Configure PrivateLink/VNet for dev/staging
**Type:** Story | **Epic Link:** E9 | **Points:** 3 | **Priority:** Low
**Labels:** `network`, `security`
**Depends on:** E1-S2 | **Links:** blocks E9-S3

**Description:** Same as E9-S1, for lower environments — may use a looser
policy than prod.

**Acceptance Criteria:**
- Network path configured and documented per environment.
